		...
		$goPricingAdmin.on('mouseenter mouseleave click', '.go-pricing-col-new-link', function(e) {
			...
			if (e.type=='mouseenter') {
				...
				$.ajax({  
					...
					if (data == 0) {
						showAjaxMsg('<p><strong>'+GoPricingL10n.ajax_error+'</strong></p>', 'error');
						return;
					}
					...
					if ($response.find('.error')) {
						showAjaxMsg($response.find('.error').html(), 'error');
						$response.remove('.error');
					}

					$response.children('.go-pricing-col').insertBefore($this);
					setColIndex();
					setSortableCols();
					$.setSytle();
					updateColCnt();
